...
import { FeeSettlementReportHelper } from '@helpers/form';
import { zodResolver } from '@hookform/resolvers/zod';
import {
  downloadFile,
  generatePacsFeeAmountReport,
  getSettlementIds
...
import { type IFeeSettlementReport } from '@typescript/form/report';
...
const feeSettlementReport = new FeeSettlementReportHelper();
const initialFileName = 'NimbaPayF';

const Pacs029FeeSettlementReport = () => {
  ...
  const schema = feeSettlementReport.schema;
  ...
  const onDownloadChangeHandler = (e: any) => {
    ...
    generatePacsFeeAmountReport(user, {
      settlementId: formData.settlementId,
      currencyId: formData.currency === 'ALL' ? 'ALL' : formData.currency,
      ...
      .then((res: any) => {
        if (res?.rptByte?.length > 0) {
          downloadFile(
            ...
            fileType,
            res.rptByte,
            `${initialFileName}-${formData.settlementId}_${moment().format('YYYYMMDD')}`
          );
        } else {
          toast({
            position: 'top',
  ...
  return (
    ...
    <VStack align="flex-start" w="full" h="full" p="3" mt={10}>
      <Stack>
        <Heading fontSize="2xl" fontWeight="bold" mb={6}>{t('ui.pacs_029_fee_settlement_report')}</Heading>
      </Stack>
...
export default memo(Pacs029FeeSettlementReport);

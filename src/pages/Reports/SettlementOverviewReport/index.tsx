...
const SettlementOverviewReport = () => {
  ...
  const currentParticipantDfspId = useMemo(() => {
      const foundParticipant = participantList?.find(
          participant => participant.participantName === user.data?.participantName
      );
      const dfspId = foundParticipant?.dfspId;
      const result = dfspId ? String(dfspId) : ''; 
      return result;
      
    }, [participantList, user.data?.participantName]);
  ...
  const onDownloadChangeHandler = (e: any) => {
    ...
    generateSettlementBankOverviewReport(user, {
        ...
        >
          ...
          >
            <Button
              ...
              isDisabled={!isValid || !participantList?.length}
              colorScheme="blue"
              gap="2"
              size="md"
              mb={2}
